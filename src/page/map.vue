...
export default {
    ...
    created () {
        ...
        //     });
        // this.$http.get('/api/user', null, r => { console.log(r); }, r => {}); // 此http请求请自行封装
    },
    mounted () {
    ...
    methods: {
        ...
        clickAction () {
            ...
            // 防止创建相同点和自交点
            if (this.points.length > 1 && ClipperLib.op_Equality(this.points[this.points.length - 1], this.hintMarker.getPosition())) { return false; }
        ...
        moveHintMarker (e) {
            ...
            if (this.prohibitSelfIntersection) { this._handleSelfIntersection(); }
        },
        setLabelText (text, styles) {
            // 提示错误信息后 2秒内不可修改标题
            if (this.updateLabelDate) {
                var now = new Date();
                if (now - this.updateLabelDate < 1000 * 2) { return; } else { this.updateLabelDate = null; }
            }
            ...
            // 节点为空 则移除提示标题
            if (this.points == null || this.points.length === 0) { this.map.removeOverlay(this.hintLabel); } else if (text) {
                // 如果有传入文本 则为错误提示标题
            ...
            } else if (this.doesSelfIntersect) { this.hintLabel.setContent('不能交叉画图'); } else if (this.points.length === 1) {
                this.map.addOverlay(this.hintLabel);
                this.hintLabel.setContent('点击继续绘制');
            } else if (this.points.length >= 2) { this.hintLabel.setContent('点击继续绘制，双击结束，右键回退，Esc键清除'); }
        },
        // 鼠标双击事件
        dblclickAction () {
        ...
        disableDraw () {
            ...
            // 清理吸附数据
            if (this.snappable) { Snapping._cleanupSnapping(); }
        ...
        _handleSelfIntersection () {
            ...
            if (this.points.length >= 3 && ClipperLib.op_Equality(this.points[this.points.length - 1], this.hintMarker.getPosition())) { paths.pop(); }
...
<style scoped>
  ...
  .map_top{
    position: absolute;
    background: rgba(14, 206, 240, 0.8);
